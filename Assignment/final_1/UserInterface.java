	...
	public static void main(String[] args) {
		...
		do {
			...
			do {
				...
				System.out.println("Total unknown type vehicles: " + myTraffic.unknownVehicleInList());
				...
				System.out.println ("\nTotal vehicles access Manual lanes:"+(myTraffic.vehicleInLaneType(V1, MC)+myTraffic.vehicleInLaneType(V2, MC)+myTraffic.vehicleInLaneType(V3, MC)));
				System.out.println ("Total vehicles access Auto lanes:"+(myTraffic.vehicleInLaneType(V1, AC)+myTraffic.vehicleInLaneType(V2, AC)+myTraffic.vehicleInLaneType(V3, AC)));
				System.out.println ("Total vehicles access e-toll lanes:"+(myTraffic.vehicleInLaneType(V1, ET)+myTraffic.vehicleInLaneType(V2, ET)+myTraffic.vehicleInLaneType(V3, ET)));
				System.out.println("Total vehicles can access the lanes: "
...
class Traffic {
		...
		// if vehicle type = V3 then count truck with more than 2 axles or truck with at least 1 trailer
			...
			for (int i = 0; i < vehicleList.length; i++) {
				if (((vehicleList[i][VEHICLE_TYPE] == TRUCK)
						&& (vehicleList[i][NUMBER_AXLES] <= 2) && (vehicleList[i][NUMBER_TRAILER] > 0))
						|| ((vehicleList[i][VEHICLE_TYPE] == TRUCK) && (vehicleList[i][NUMBER_AXLES] > 2))) {
					count++;
				}
			}
		}
	...
	long unknownVehicleInList(){
		long unknownVehicle = vehicleInList()-(vehicleInList(V1)+vehicleInList(V2)+vehicleInList(V3));
